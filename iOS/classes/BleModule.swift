    ...
    public func connectToDevice(_ deviceIdentifier: String,
        ...
        if let options = options {
            ...
            if #available(iOS 13.0, *) {
                if let isEnableTransportBridging = options["enableTransportBridging"] as? Bool {
                    connectOptions[CBConnectPeripheralOptionEnableTransportBridgingKey] = isEnableTransportBridging as AnyObject
                }
                
                if let isRequiresANCS = options["requiresANCS"] as? Bool {
                    connectOptions[CBConnectPeripheralOptionRequiresANCS] = isRequiresANCS as AnyObject
                }
            }
            
            if let startDelay = options["startDelay"] as? Int {
                connectOptions[CBConnectPeripheralOptionStartDelayKey] = NSNumber(value: startDelay)
            }
        }
